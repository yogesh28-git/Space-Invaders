...
namespace UI
{
	namespace UIElement
	{
		...
		{
			float alpha = std::max(0.0f, 1 - (elapsed_duration / animation_duration));
			image_sprite.setColor(sf::Color(255, 255, 255, static_cast<sf::Uint8>(alpha * 255)));
		}
		...
		void AnimatedImageView::playAnimation(AnimationType animation_type, float duration, CallbackFunction animation_end_callback)
		{
			ImageView::show();
			reset();
			setAnimationType(animation_type);
			setAnimationDuration(duration);
			registerCallbackFunction(animation_end_callback);
		}
		void AnimatedImageView::registerCallbackFunction(CallbackFunction animation_end_callback)
		{
